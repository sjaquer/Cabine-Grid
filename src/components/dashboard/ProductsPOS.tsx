  ...
  machineName = "PC",
        ...
        return (
            ...
            >
                <div className="flex-1 min-w-0 pr-2 select-none">
                    <span className="text-foreground font-semibold text-sm block truncate tracking-tight">
                        {product.name}
                    </span>
                    <div className="flex items-center gap-2 mt-1">
                        ...
                        {availableStock > 0 && (
                            <Badge variant="outline" className="text-xs font-mono border-border bg-background text-muted-foreground py-0 px-1.5">
                ...
                <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    {currentQty > 0 ? (
                        <div className="flex items-center gap-1.5 bg-background border border-border p-1.5 rounded-xl shadow-inner">
                            <Button
                                variant="ghost"
                                size="icon"
                                className="h-8 w-8 text-muted-foreground hover:text-foreground hover:bg-secondary disabled:opacity-30 rounded-lg"
                                onClick={() => handleQuantityChange(product.id, -1)}
                            >
                                <MinusCircle className="w-4 h-4" />
                            </Button>
                            <span className="text-primary font-mono font-bold text-sm min-w-[24px] text-center">
                                {currentQty}
                            </span>
                            <Button
                                variant="ghost"
                        ...
                        <Button
                            variant="ghost"
                            size="icon"
                            className="h-8 w-8 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded-xl border border-border hover:border-primary/30 transition-all flex items-center justify-center"
                            onClick={() => handleQuantityChange(product.id, 1)}
                        >
                            <PlusCircle className="w-4 h-4" />
                        </Button>
    ...
    return (
        <div className="flex h-full min-h-0 flex-col bg-background text-foreground font-body overflow-hidden">
            ...
            <div className="mt-auto border-t border-border bg-card/90 p-3 shadow-[0_-8px_20px_hsl(var(--background)/0.35)] space-y-3 shrink-0">
                
                {activeSession && costCalculation && (
                    <div className="p-2.5 rounded-xl bg-secondary/35 border border-border/60 space-y-1.5 text-xs">
                        <div className="flex justify-between items-center">
                            <span className="text-muted-foreground font-semibold uppercase tracking-wide text-xs">Tiempo:</span>
                            <span className="font-mono font-bold text-foreground bg-background px-2 py-0.5 rounded-md border border-border shadow-sm">
                                {formatTime(Math.floor(costCalculation.elapsedSeconds))}
                            </span>
                        </div>
                        <div className="flex justify-between items-center">
                            <span className="text-muted-foreground font-semibold uppercase tracking-wide text-xs">Costo:</span>
                ...
                {activeSession && (
                    <div className="space-y-2 p-2.5 rounded-xl bg-secondary/35 border border-border/60">
                        <Label className="text-xs uppercase tracking-wide font-semibold text-muted-foreground">Medio de Pago</Label>
                        <RadioGroup
                            value={paymentMethod}
                            onValueChange={(val) => setPaymentMethod(val as PaymentMethod)}
                            className="grid grid-cols-3 gap-1"
                        >
                            <Label htmlFor="efectivo" className="flex flex-col items-center justify-center gap-1 rounded-lg border border-border bg-background p-2 cursor-pointer transition-all hover:bg-secondary [&:has([data-state=checked])]:border-primary [&:has([data-state=checked])]:bg-primary/10">
                                <RadioGroupItem value="efectivo" id="efectivo" className="sr-only" />
                                <span className="text-xs font-bold">Efectivo</span>
                            </Label>
                            <Label htmlFor="yape" className="flex flex-col items-center justify-center gap-1 rounded-lg border border-border bg-background p-2 cursor-pointer transition-all hover:bg-secondary [&:has([data-state=checked])]:border-info [&:has([data-state=checked])]:bg-info/10">
                                <RadioGroupItem value="yape" id="yape" className="sr-only" />
                                <span className="text-xs font-bold">Yape/Plin</span>
                            </Label>
                            <Label htmlFor="otro" className="flex flex-col items-center justify-center gap-1 rounded-lg border border-border bg-background p-2 cursor-pointer transition-all hover:bg-secondary [&:has([data-state=checked])]:border-status-warning [&:has([data-state=checked])]:bg-status-warning/10">
                        ...
                        {paymentMethod === 'efectivo' && (
                            <div className="space-y-2 p-2 bg-background border border-border/60 rounded-lg">
                                <div className="flex justify-between items-center">
                                    <Label htmlFor="amount-paid" className="text-xs font-semibold uppercase text-muted-foreground tracking-wide">Recibido</Label>
                                    <Input
                                        id="amount-paid"
                                        type="number"
                                        min={0}
                                        ...
                                        className="text-right font-mono font-bold h-8 border-border bg-card text-foreground text-xs w-24 focus-visible:ring-primary"
                                    />
                                </div>
                                {change > 0 && (
                                    <div className="flex justify-between items-center pt-1 border-t border-border">
                                        <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Vuelto:</span>
                ...
                {soldProducts.length > 0 ? (
                    <div className="max-h-24 overflow-y-auto space-y-1.5 pr-1 text-xs text-muted-foreground">
                        {soldProducts.map((sp) => (
                            <div key={sp.productId} className="flex items-center justify-between border-b border-border/40 pb-1.5 last:border-0 last:pb-0">
                                <span className="truncate max-w-[200px]">{sp.productName}</span>
                                <div className="flex items-center gap-2 font-mono">
                                    <div className="flex items-center gap-1 bg-card px-1.5 py-0.5 rounded-md border border-border">
                                        <button
                                        ...
                                        <span className="text-foreground font-bold text-xs min-w-[16px] text-center">{sp.quantity}</span>
                                        <button
                                            className="text-muted-foreground hover:text-foreground transition-colors disabled:opacity-30"
                                            onClick={() => handleQuantityChange(sp.productId, 1)}
                                        >
                                    ...
                                    <span className="font-bold text-primary text-xs ml-1">{formatCurrency(sp.quantity * sp.unitPrice)}</span>
                                </div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="rounded-lg border border-dashed border-border/70 bg-background/40 p-2 text-xs text-muted-foreground">Sin productos en carrito</div>
                )}
                ...
                {appliedDiscount > 0 && (
                    <div className="space-y-1 text-xs border border-primary/30 bg-primary/10 rounded-lg p-2">
                        <div className="flex items-center justify-between">
                            <span className="text-foreground">Ingreso Bruto</span>
                            <span className="font-mono text-foreground">{formatCurrency(grossTotal)}</span>
                        </div>
                        <div className="flex items-center justify-between">
                            <span className="text-foreground">Descuento Aplicado</span>
                            <span className="font-mono text-status-warning">- {formatCurrency(appliedDiscount)}</span>
                        </div>
                        <div className="flex items-center justify-between border-t border-border pt-1">
                            <span className="text-foreground font-semibold">Ingreso Real</span>
                ...
                {activeSession?.clientId && customerCards && customerCards.length > 0 && (
                    <div className="space-y-2 p-2 rounded-lg border border-border/40 bg-card/30">
                        <div className="flex items-center justify-between">
                            <span className="text-xs font-bold text-foreground">Cartas Disponibles</span>
                        </div>
                        <div className="grid gap-2">
                            {customerCards.map((c) => (
                                <div key={c.id} className="flex items-center justify-between p-2 bg-background/40 border border-border/40 rounded">
                                    <div className="text-xs">
                                        <div className="font-semibold">{c.name}</div>
                                        <div className="text-xs text-muted-foreground">{c.type} {c.value ? `• ${c.value}` : ''}</div>
                                    </div>
                                    <div>
                ...
                {activeSession && onConfirmPayment && (
                    <Button 
                        ...
                        className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-sm py-3.5 rounded-xl transition-all"
                    >
                ...
                {activeSession && onConfirmPayment && !canRegisterDebt && (
                    <p className="text-xs text-status-warning border border-status-warning/30 bg-status-warning/10 rounded-lg p-2">Para deuda, vincula cliente CRM.</p>
                )}
                ...
                <div className="flex gap-2 pt-1">
                    {onClose && (
                        <Button 
                            ...
                            onClick={onClose} 
                            disabled={isSaving}
                            className="flex-1 h-10 text-xs font-semibold text-muted-foreground hover:bg-secondary border border-border"
                        >
                            Cerrar (Esc)
                        </Button>
                    )}
                    <Button 
                        ...
                        disabled={isSaving}
                        className="flex-[2] h-10 bg-secondary text-secondary-foreground hover:bg-secondary/80 font-semibold text-xs border border-border"
                    >
