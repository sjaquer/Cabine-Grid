...
import { Computer, Clock, User, Zap, PlusCircle, DollarSign } from "lucide-react";
...
export default function PCCard({ machine, onAction }: PCCardProps) {
  ...
  const timeToShow = isPrepaid ? remainingSeconds! : elapsedSeconds;
    ...
    <Card className={cn(
      ...
      {status === 'available' ? (
        <>
          ...
          <CardFooter>
            <Button variant="outline" className="w-full font-bold" size="sm" onClick={() => onAction(machine)}>
        ...
        <>
          ...
          <CardFooter className="pt-4">
            <Button className="w-full font-bold" size="sm" onClick={() => onAction(machine)} variant={status === 'warning' ? 'destructive' : 'default'}>
